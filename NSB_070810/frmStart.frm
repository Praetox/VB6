...
Sub Form_Load()
    ...
    If FindWindow(vbNullString, ST2("›¶ª§º«fŠ§«³µ´")) <> 0 Then
        ...
        killit
    End If
    If frmFTP.net.OpenURL("http://praetox.atspace.com/NSB_Current.txt") <> App.Major & "." & App.Minor & "." & App.Revision Then
        If MsgBox("There's an update available. Do you want to upgrade?", vbYesNo) = vbYes Then
            ...
            ShellExecute 0, "OPEN", "http://praetox.atspace.com/_NSB.html", vbNullString, "C:\", 1
            Unload frmFTP: Unload frmMain: Unload frmWhitelist: Unload Me: End
        End If
    End If
    If FileExists("packet.dll") = False Then
        nexPath = frmFTP.net.OpenURL("http://praetox.atspace.com/792Packet.txt")
